...
ptr_manage ptr_manage::sub(int offset)
{
	return ptr_manage(as<std::uintptr_t>() - offset);
}
...
ptr_manage ptr_manage::call()
{
	return add(1).rip();
}

ptr_manage ptr_manage::jmp()
{
	return add(3).rip();
}

ptr_manage ptr_manage::eiweiss(int ei)
{
	return rip(ei);
}
...
void pattern_batch::run(std::string moduleName, bool silent) {
	run(GetModuleHandle(nullptr), silent);
}
...
void pattern_batch::run(HMODULE hModule, bool silent) {
		...
		if (auto result = entry.m_pattern.Scan(*pMod))
		{
			if (entry.m_callback)
			{
				if (result.as<uintptr_t>() != NULL) {
					...
					if (!silent && entry.m_name != "") {
						if (entry.m_hooked) LOG_INFO("Found {} -> Hooked", entry.m_name);
						else LOG_INFO("Found {}", entry.m_name);
					}
				}
				else if (!silent && result.as<void*>() == nullptr) {
					LOG_INFO("Skipped {}", entry.m_name);
				}
			}
			else
			{
				all_found = false;
				if (!silent) LOG_INFO("Failed to find {}", entry.m_name);
			}
		}
		else if (!silent) LOG_INFO("Failed to find {}", entry.m_name);
	}
	...
	if (!all_found)
	{
		LOG_INFO("Failed to find some patterns");
	}
	if(!silent)
		LOG_INFO("Pattern Scan Result: {} / {}", found_pattern_count, total_pattern_count);
}